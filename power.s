...
_start:
  ...
  popl %ebx               #the second answer is already
                          ...
                          #out into %ebx
  addl %eax, %ebx         #add them togather
  ...
  power:
    ...
    subl $4, %esp         #get room for our storage

    movl 8(%ebp), %ebx    #put first arguemnt in %ebx
  ...
  power_loop_start:
    ...
    je end_power
    movl -4(%ebp), %eax   #move the current result int %eax
    imull %ebx, %eax      #multiply current result by
                          #the base number
  ...
  end_power:
    ...
    movl %ebp, %esp       #restore the stack ponter
